...
class Tester {
  ...
  private function runTest(string $name, int $level = 0) {
    $src = "./ipp-2023-tests/$name.src";
    $in = "./ipp-2023-tests/$name.in";
    $out = "./ipp-2023-tests/$name.out";

    $cmd = "{$this->php} interpret.py --source=$src --input=$in > tmp/out.tmp 2>/dev/null";
    $output = null;
    $rc = null;
    exec($cmd, $output, $rc);

    // test return code
    $expectedRc = intval(file_get_contents("./ipp-2023-tests/$name.rc"));
    if ($rc !== $expectedRc) {
    ...
    if ($rc === 0) {
      ...
      echo "Got:\n";
      echo file_get_contents("tmp/out.tmp") . "\n";
    }
  }
  ...
  private function testDir(string $path, int $level) {
    ...
    $dir = opendir("./ipp-2023-tests/$path");
    while ($file = readdir($dir)) {
      if ($file === "." || $file === "..") continue;
      $filePath = "./ipp-2023-tests/$path/$file";
      if (is_dir($filePath)) {
        $prefix = $path === "" ? "" : "$path/";
  ...
  function runTests(string $start = "") {
    ...
    if ($start !== "") {
      if (is_dir("./ipp-2023-tests/$start")) {
        $this->testDir($start, 0);
      } else if (is_file("./ipp-2023-tests/$start.src")) {
